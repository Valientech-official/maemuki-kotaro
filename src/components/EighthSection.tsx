...
export default function EighthSection() {
  ...
  const [faqItems, setFaqItems] = useState<FAQItem[]>([
    ...
    },
    {
      ...
      answer:
        "お客様のご状況により異なりますが、弊社のサポートを受けている方だと5〜7倍くらい増える方が多いです。",
      isOpen: false,
    },
    {
    ...
    },
    {
      ...
      answer:
        "自己都合退職でも問題ございません。自己都合でも会社都合でも、手続きや給付のスケジュールは同じです。",
      isOpen: false,
    },
    {
      question: "いつから受給が開始されますか？",
      answer:
        "お客様のご状況によりますが、最短約1ヶ月で受給が開始されます。通常の失業保険では3ヶ月の給付制限期間がありますが、適切な手続きにより短縮が可能です。",
      isOpen: false,
    },
    {
      question: "退職給付金は何ヶ月もらえるんですか？",
      answer:
        "お客様のご状況により異なりますが、弊社のサポートを受けている方だと最大28〜30ヶ月くらいになる場合が多いです。",
      isOpen: false,
    },
    {
      question: "手続きが複雑で不安です",
      answer:
        "ご安心ください。弊社では専門スタッフが申請手続きを完全サポートいたします。必要書類の準備から申請まで、お客様に代わって対応いたします。",
      isOpen: false,
    },
    {
      question: "相談や診断に費用はかかりますか？",
      answer:
        "初回相談と受給額診断は完全無料です。LINEでお気軽にご相談いただけます。サポートが必要な場合のみ、成果報酬制でご利用いただけます。",
      isOpen: false,
    },
    {
      question: "どのような方がサポートを受けていますか？",
      answer:
        "会社員、公務員、契約社員など様々な職種の方にご利用いただいています。転職を検討されている方、すでに退職された方、どちらでもサポート可能です。",
      isOpen: false,
    },
  ]);
        ...
        >
          ...
          >
            {faqItems.map((item, index) => (
              <div
                key={index}
                className="faq-item border-b border-orange-100 last:border-b-0"
              >
                <div
                ...
                >
                  <div className="content-stretch flex gap-3 items-start justify-start relative flex-1 min-w-0">
                    <div
                      className={`${
                        item.isOpen ? "bg-[#e46421]" : "bg-[#f97316]"
                      } content-stretch flex items-center justify-center relative rounded-[18px] shrink-0 size-9 transition-colors duration-300`}
                    >
                    ...
                    <div className="flex flex-col font-bold justify-center leading-[0] relative flex-1 min-w-0 text-[14px] text-slate-800 tracking-[0.7px]">
                      <p className="leading-[22.4px] break-words overflow-hidden">
                        {item.question}
                      </p>
                    </div>
                  </div>
                  <div className="relative shrink-0 size-4">
                    <img
                      ...
                      className={`block max-w-none size-full transition-transform ${
                        item.isOpen ? "rotate-45" : "rotate-0"
                      }`}
                      src={imgVector}
                    />
                  </div>
                ...
                <div
                  id={`faq-answer-${index}`}
                  className={`bg-gradient-to-r from-orange-50 to-amber-50 overflow-hidden ${
                    !item.isOpen ? "h-0 opacity-0" : ""
                  }`}
                >
                  {item.answer && (
